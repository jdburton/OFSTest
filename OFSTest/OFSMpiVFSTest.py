...
def IOR(testing_node,output=[]):
    ...
    rc = testing_node.runSingleCommand("%s/bin/mpiexec -np %s --machinefile %s --map-by node --mca btl_tcp_if_include eth0 %s/src/C/IOR -a POSIX -C -F -i 4 -N %s -b 2g -t 2m -s 1 -o %s/mpivfsfile" % (testing_node.openmpi_installation_location,np,testing_node.created_openmpihosts,testing_node.ior_installation_location,np,testing_node.ofs_mount_point),output)
...
tests = [ romio_testsuite, mpi_active_delete, IOR, mdtest, miranda_io, simul,multi_md_test, multi_md_test_size_sweep ]
